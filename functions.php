...
if ( ! function_exists( 'vlog_setup' ) ) :
	...
	function vlog_setup() {
		...
		register_nav_menus(
			array(
				'menu-1' => esc_html__( 'Primary', 'vlog' ),
			)
		);
...
function vlog_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'vlog_content_width', 640 );
}
...
function vlog_scripts() {
	wp_enqueue_style( 'vlog-style', get_stylesheet_uri(), array(), _S_VERSION );
	wp_style_add_data( 'vlog-style', 'rtl', 'replace' );

	wp_enqueue_script( 'vlog-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true );
